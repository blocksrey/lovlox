...
function XmlParser.new(_handler)
 local obj = {
 handler = _handler,
 _stack = {}
 }
...
local function parseTag(self, s)
 ...
 gsub(s, self._ATTR1, parseFunction) 
 gsub(s, self._ATTR2, parseFunction)
...
local function parseXmlDeclaration(self, xml, f)
 -- XML Declaration
 f.match, f.endMatch, f.text = find(xml, self._PI, f.pos)
 if not f.match then 
...
local function parseXmlProcessingInstruction(self, xml, f)
 ...
 local pi = sub(f.text, len(tag.name)+1)
 if pi ~= "" then
 if tag.attrs then
 tag.attrs._text = pi
 else
...
local function parseComment(self, xml, f)
 f.match, f.endMatch, f.text = find(xml, self._COMMENT, f.pos)
 if not f.match then 
...
local function _parseDtd(self, xml, pos)
 ...
 local m,e,r,t,n,u,i = find(xml, dtd, pos)
 if m then
...
local function parseDtd(self, xml, f)
 ...
 local tag = {name="DOCTYPE", value=sub(xml, f.match+10, f.endMatch-1)}
 self.handler:dtd(tag, f.match, f.endMatch)
 end
end

local function parseCdata(self, xml, f)
 f.match, f.endMatch, f.text = find(xml, self._CDATA, f.pos)
 if not f.match then 
...
local function parseNormalTag(self, xml, f)
 ...
 f.errStart, f.errEnd = find(f.tagstr, self._ATTRERR1) 

 if f.errEnd == nil then
 f.errStart, f.errEnd = find(f.tagstr, self._ATTRERR2)
 if f.errEnd == nil then
 break
 end
 end
 
 f.extStart, f.extEnd, f.endt2 = find(xml, self._TAGEXT, f.endMatch+1)
 f.tagstr = f.tagstr .. sub(xml, f.endMatch, f.extEnd-1)
 if not f.match then 
 ...
 err(self, format("%s (/%s)", self._errstr.endTagErr, tag.name), f.pos)
 end
 if table.remove(self._stack) ~= tag.name then
 err(self, format("%s (/%s)", self._errstr.unmatchedTagErr, tag.name), f.pos)
 end
...
local function parseTagType(self, xml, f)
 -- Test for tag type
 if find(sub(f.tagstr, 1, 5), "?xml%s") then
 parseXmlDeclaration(self, xml, f)
 elseif sub(f.tagstr, 1, 1) == "?" then
 parseXmlProcessingInstruction(self, xml, f)
 elseif sub(f.tagstr, 1, 3) == "!--" then
 parseComment(self, xml, f)
 elseif sub(f.tagstr, 1, 8) == "!DOCTYPE" then
 parseDtd(self, xml, f)
 elseif sub(f.tagstr, 1, 8) == "![CDATA[" then
...
local function getNextTag(self, xml, f)
 f.match, f.endMatch, f.text, f.endt1, f.tagstr, f.endt2 = find(xml, self._XML, f.pos)
 if not f.match then 
 if find(xml, self._WS, f.pos) then
...
function XmlParser:parse(xml, parseAttributes)
 ...
 --Stores find results and parameters
 ...
 --find parameters and auxiliar variables
 pos = 1,
 -- startText, endText,
 ...
 -- Handle leading text
 f.startText = f.match
 f.endText = f.match + len(f.text) - 1
 f.match = f.match + len(f.text)
 f.text = parseEntities(self, stripWS(self, f.text))
